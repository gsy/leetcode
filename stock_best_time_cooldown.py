...
class Solution(object):
    def maxProfit(self, prices):
        ...
        for j in range(1, length):
            ...
            if price >= last:
                if price - minimum > local_best:
                    local_best = price - minimum
            else:
                if j >= 2 and prices[j - 1] > prices[j - 2]:
                    cooldown_one_day_earlier_cost = prices[j-1] - prices[j-2]
                else:
                    cooldown_one_day_earlier_cost = 0
                cooldown_this_day_cost = prices[j+1] - prices[j] if j + 1 < length else 0

                if cooldown_one_day_earlier_cost < cooldown_this_day_cost:
                    local_best = prices[j-2] - minimum
                    minimum = prices[j]
                else:
                    local_best = prices[j-1] - minimum
                    if j + 1 < length:
                        minimum = prices[j+1]
